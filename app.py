...
def generate_clean_tmx(previews: dict, results: dict, originals: dict,
    ...
    for filename, file_previews in previews.items():
        for preview in file_previews:
            ...
            prop_confirmed = etree.SubElement(tu, "prop", type="x-confirmedcount")
            prop_confirmed.text = "1"
            
            now_tmx = datetime.now().strftime("%Y%m%dT%H%M%SZ")
            
            tuv_src = etree.SubElement(tu, "tuv")
            tuv_src.set("{http://www.w3.org/XML/1998/namespace}lang", src_lang)
            tuv_src.set("changedate", now_tmx)
            tuv_src.set("creationdate", now_tmx)
            ...
            tuv_tgt.set("changedate", now_tmx)
            tuv_tgt.set("creationdate", now_tmx)
...
def generate_changes_excel(dedup_details: list = None, exclusion_breakdown: dict = None) -> bytes:
    wb = Workbook()
    ws = wb.active
