...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("")
        st.markdown("#### Filters")
        filter_junk = st.checkbox("Exclude short segments", value=True, help="Removes junk segments: <3 words or only numbers/symbols")
        st.session_state['filter_junk'] = filter_junk
        filter_short_segments = st.checkbox("Exclude short anonymized segments", value=True, help="Excludes short anonymized segments")
        min_words = st.slider("Min. words", 2, 10, 5, 1) if filter_short_segments else 5
        ...
        if "previews" in st.session_state and st.session_state.previews:
            ...
            for filename, file_previews in st.session_state.previews.items():
                ...
                if filtered_previews:
                    total_matches += len(filtered_previews)
                    with st.expander(f"📄 {filename} ({len(filtered_previews)} affected segments)", expanded=True):
    ...
    with tab3:
        if "results" in st.session_state and st.session_state.results:
            ...
            def get_junk_segment_keys(filename: str, file_previews: list) -> set:
                ...
                for preview in file_previews:
                    if is_junk_segment(preview):
                        junk_keys.add(f"{filename}_{preview['segment']}")
                return junk_keys
            ...
            if filter_short:
                short_count = sum(
                    1 for file_previews in previews.values()
                    for preview in file_previews
                    if not (filter_junk and is_junk_segment(preview)) and segment_word_count(preview) < min_words
                )
                if short_count > 0:
                    st.markdown(f"""
