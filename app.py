...
def generate_changes_excel(dedup_details: list = None, exclusion_breakdown: dict = None, tmx_traceability: list = None) -> bytes:
    wb = Workbook()
    ws = wb.active
    ...
    for item in tmx_traceability:
        for col, value in enumerate([item["file"], item["segment"], item["source"], item["target"], item["date"]], 1):
