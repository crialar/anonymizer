                    ...
                    with st.expander(f"📄 {filename} ({', '.join(label_parts)} segments)", expanded=True):
                        for preview in filtered_previews:
                            ...
                            if is_no_anon:
                                ...
                                segment_header += f' <span style="background:#c8c8c8;color:#333333;padding:2px 6px;border-radius:4px;font-size:0.75rem;">{max_pct:.0f}% anonymized</span>'
        ...
        if "results" in st.session_state and st.session_state.results:
            ...
            if not dedup_tmx:
                ...
                if not conflict_groups:
                    ...
                    st.markdown(f"<small>Select which segment to keep in each match. Segments with similar source (≥{dedup_threshold}% threshold applied to source only) but different translations.</small>", unsafe_allow_html=True)
