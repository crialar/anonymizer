...
def is_junk_segment(preview: dict) -> bool:
...
def render_stat_card(label: str, value: int, col):
    with col:
        st.markdown(f"""
        <div class="stat-card">
...
def generate_changes_excel() -> bytes:
    wb = Workbook()
    ws = wb.active
    ...
    ws['A1'] = "ANONYMIZATION REPORT - MQXLIFF/TMX Anonymizer v6.1"
    ...
    row += 1
    stats_headers = ["File", "Safe Regex", "Regex CT IDs", "Presidio", "ScispaCy", "Dictionary", "Total"]
    ...
    for filename, stats in st.session_state.all_stats.items():
        file_total = stats.get("safe_regex", 0) + stats.get("regex_ct", 0) + stats.get("presidio_pii", 0) + stats.get("biomedical", 0) + stats.get("dictionary", 0)
        grand_total += file_total
        data = [filename, stats.get("safe_regex", 0), stats["regex_ct"], stats["presidio_pii"], stats["biomedical"], stats["dictionary"], file_total]
    ...
    for filename, file_previews in st.session_state.previews.items():
        for preview in file_previews:
            if filter_junk and is_junk_segment(preview):
                continue
    ...
    ws2['A1'] = "EXCLUDED SEGMENTS - Anonymizer v6.1"
    ...
    replacement_token = st.session_state.get('replacement_token', 'REDACTED')
    ...
    if has_junk:
        ...
        ws2[f'A{ex_row}'] = "JUNK SEGMENTS - Excluded from TM (<3 words or only numbers/symbols)"
        ...
        if junk_count == 0:
            ws2.cell(row=ex_row, column=1, value="No junk segments found.").font = Font(italic=True, color="666666")
            ex_row += 1
        else:
            ws2.merge_cells(f'A{ex_row}:F{ex_row}')
            ws2[f'A{ex_row}'] = f"Total: {junk_count} junk segments excluded from TM"
    ...
    if has_short:
        ...
        for filename, file_previews in st.session_state.previews.items():
            for preview in file_previews:
                if has_junk and is_junk_segment(preview):
                    continue
                if not preview.get('changed', True):
                    continue
    ...
    if not has_junk and not has_short and not has_tm:
...
def main():
    st.markdown("""
    <div style="background: linear-gradient(135deg, #1a5488 0%, #0e7bc0 100%); 
                padding: 2rem; 
                border-radius: 12px; 
                margin-bottom: 2rem;
        ...
        <h1 style="color: white !important; margin: 0; font-size: 2.5rem; font-weight: 700; letter-spacing: 8px;">
            Anonymizer <span style="background: rgba(255,255,255,0.2); color: rgba(255,255,255,0.95); font-size: 0.9rem; font-weight: 500; padding: 3px 10px; border-radius: 20px; vertical-align: middle; letter-spacing: 1px; border: 1px solid rgba(255,255,255,0.3);">v6.1</span>
        </h1>
        <p style="color: rgba(255,255,255,0.9) !important; margin: 0.5rem 0 0 0; font-size: 1.1rem;">
            Anonymize bilingual memoQ documents (EN > ES)
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        replacement_token = st.text_input("Replacement token", value="REDACTED")
        st.session_state['replacement_token'] = replacement_token
        process_source = True
        process_target = True
        st.markdown("")
        st.markdown("#### Anonymization layers")
        use_safe_regex = st.checkbox("Safe Regex", value=True, help="High-precision patterns: emails, phones, URLs, etc.")
        ...
        use_dictionary = st.checkbox("Custom dictionary", value=True, help="Manually loaded sensitive terms")
        st.markdown("")
        st.markdown("#### Filters")
        filter_junk = st.checkbox("Exclude short segments", value=True, help="Removes junk segments: less than 3 words or only numbers/symbols")
        ...
        min_words = st.slider("Min. words (anonymized)", 2, 10, 5, 1) if filter_short_segments else 5
        ...
        if mqxliff_files:
            ...
            if st.button("🚀 Process files", type="primary", use_container_width=True):
                process_files(
                    ...
                    use_safe_regex, use_regex, use_presidio, use_biomedical, use_dictionary,
                    dictionary_terms, whitelist_terms
                )
    ...
    with tab2:
        if "previews" in st.session_state and st.session_state.previews:
            ...
            for filename, file_previews in st.session_state.previews.items():
                filtered_previews = []
                for preview in file_previews:
                    is_junk = is_junk_segment(preview)
                ...
                if filtered_previews:
                    ...
                    if junk_in_file > 0:
                        label_parts.append(f"{junk_in_file} junk")
                            ...
                            if preview_is_junk:
                                ...
                                with col_header:
                                    if junk_skipped:
                                        ...
                                        st.markdown(
                                            ...
                                            f'<strong>Segment {preview["segment"]}</strong> '
                                            f'<span style="background:#6c757d;color:white;padding:2px 6px;border-radius:4px;font-size:0.75rem;">🗑️ Junk</span>'
                                            f'<br><small>EN: {preview["source_before"][:80] or "(empty)"} | ES: {preview["target_before"][:80] or "(empty)"}</small>'
                                            f'</div>',
                                            unsafe_allow_html=True
                                        )
                                with col_skip_junk:
                                    ...
                                    skip_junk = st.checkbox(
                                        "Skip",
                                        key=cb_key,
                                        help="Keep this segment (don't treat as junk)"
                                    )
            ...
            if filter_junk and junk_segments_count > 0:
                st.info(f"🗑️ {junk_segments_count} junk segments excluded from TM (<3 words or only numbers/symbols)")
    ...
    with tab3:
        if "results" in st.session_state and st.session_state.results:
            ...
            total_stats = {"safe_regex": 0, "regex_ct": 0, "presidio_pii": 0, "biomedical": 0, "dictionary": 0}
            for stats in st.session_state.all_stats.values():
                for key in total_stats:
                    total_stats[key] += stats.get(key, 0)
            
            col1, col2, col3, col4, col5 = st.columns(5)
            render_stat_card("Safe Regex", total_stats["safe_regex"], col1)
            render_stat_card("Regex CT IDs", total_stats["regex_ct"], col2)
            render_stat_card("Presidio", total_stats["presidio_pii"], col3)
            render_stat_card("Biomedical", total_stats["biomedical"], col4)
            render_stat_card("Dictionary", total_stats["dictionary"], col5)
            ...
            if exclude_modified_targets:
                ...
                def calc_pct(text, token):
                    if not text:
                        return 0
                    words = text.split()
                    if not words:
                        return 0
                    return sum(1 for w in words if token in w) / len(words) * 100
                
                excluded_count = 0
                for filename, file_previews in st.session_state.get('previews', {}).items():
                    for preview in file_previews:
                        target_pct = calc_pct(preview.get('target_after', ''), replacement_token)
                        if target_pct >= exclusion_threshold:
                            segment_key = f"{filename}_{preview['segment']}"
                            if excluded_segments.get(segment_key, True):
                                excluded_count += 1
                
                if excluded_count > 0:
                    st.markdown(f"""
                    <div style="background-color: #fff3cd; border: 1px solid #ffc107; border-radius: 8px; padding: 1rem; margin: 0.5rem 0;">
                        <strong>⚠️ {excluded_count} segments will be excluded from TM</strong><br>
                        <small>Threshold: {exclusion_threshold}% | You can uncheck them in the Preview tab</small>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.info("No segments marked for exclusion")
            
            def get_junk_segment_keys(filename: str, file_previews: list) -> set:
            ...
            if filter_junk:
                ...
                for fname, file_previews in previews.items():
                    for preview in file_previews:
                        if is_junk_segment(preview):
                            sk = f"{fname}_{preview['segment']}"
                            if not st.session_state.get(f"skipjunk_{sk}", False):
                                junk_count += 1
                if junk_count > 0:
                    st.markdown(f"""
                    <div style="background-color: #e2e3e5; border: 1px solid #6c757d; border-radius: 8px; padding: 1rem; margin: 0.5rem 0;">
                        <strong>🗑️ {junk_count} junk segments excluded from TM</strong><br>
                        <small>&lt;3 words or only numbers/symbols</small>
                    </div>
                    """, unsafe_allow_html=True)
            
            if filter_short:
                short_count = sum(
                    1 for file_previews in previews.values()
                    for preview in file_previews
                    if preview.get('changed', True) and not (filter_junk and is_junk_segment(preview)) and segment_word_count(preview) < min_words
                )
                if short_count > 0:
                    st.markdown(f"""
                    <div style="background-color: #e8f4fc; border: 1px solid #0e7bc0; border-radius: 8px; padding: 1rem; margin: 0.5rem 0;">
                        <strong>✂️ {short_count} short anonymized segments excluded from TM</strong><br>
                        <small>Less than {min_words} words → anonymized but excluded from TM update</small>
                    </div>
                    """, unsafe_allow_html=True)
            
            col_mqxliff, col_excel = st.columns(2)
            
            with col_mqxliff:
                if len(st.session_state.results) == 1:
                    filename, content = list(st.session_state.results.items())[0]
                    orig_content = originals.get(filename, content)
                    junk_segs = get_junk_segment_keys(filename, previews.get(filename, [])) if filter_junk else set()
                    ...
                    file_ext = filename.rsplit(".", 1)[-1].upper() if "." in filename else "XML"
                    st.download_button(
                    ...
                    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                        for filename, content in st.session_state.results.items():
                            orig_content = originals.get(filename, content)
                            junk_segs = get_junk_segment_keys(filename, previews.get(filename, [])) if filter_junk else set()
                            short_segs = get_short_segment_keys(filename, previews.get(filename, []), min_words) if filter_short else set()
                            content = apply_no_anon_segments(content, orig_content, filename, no_anon_segments)
                            download_content = prepare_download_content(content, filename, exclude_modified_targets, exclusion_threshold, exclude_source_too, excluded_segments, short_segs, junk_segs)
                            zf.writestr(filename, download_content)
            ...
            with col_excel:
                ...
                st.download_button(
                    label="📊 Download changes report (Excel)",
                    data=excel_data,
...
def process_files(files, replacement_token, process_source, process_target,
                  use_safe_regex, use_regex, use_presidio, use_biomedical, use_dictionary,
                  dictionary_terms, whitelist_terms=None):
    ...
    for i, file in enumerate(files):
        progress_bar.progress((i + 1) / len(files), text=f"Processing {file.name}...")
