...
def highlight_diff(text_a: str, text_b: str) -> tuple:
    ...
    for tag, i1, i2, j1, j2 in sm.get_opcodes():
        chunk_a = _html_escape(" ".join(words_a[i1:i2]))
        chunk_b = _html_escape(" ".join(words_b[j1:j2]))
        if tag == 'equal':
            html_a_parts.append(chunk_a)
            html_b_parts.append(chunk_b)
        elif tag == 'replace':
            html_a_parts.append(f'<span style="background:#ffcccc;color:#c00;font-weight:600;padding:0 2px;border-radius:2px;">{chunk_a}</span>')
            html_b_parts.append(f'<span style="background:#ffcccc;color:#c00;font-weight:600;padding:0 2px;border-radius:2px;">{chunk_b}</span>')
        elif tag == 'delete':
            html_a_parts.append(f'<span style="background:#ffcccc;color:#c00;font-weight:600;padding:0 2px;border-radius:2px;">{chunk_a}</span>')
        elif tag == 'insert':
            html_b_parts.append(f'<span style="background:#ffcccc;color:#c00;font-weight:600;padding:0 2px;border-radius:2px;">{chunk_b}</span>')
                    ...
                    with st.expander(f"📄 {filename} ({', '.join(label_parts)} segments)", expanded=True):
                        for preview in filtered_previews:
                            ...
                            if is_no_anon:
                                ...
                                segment_header += f' <span style="background:#5a6268;color:white;padding:2px 6px;border-radius:4px;font-size:0.75rem;">{max_pct:.0f}% anonymized</span>'
        ...
        if "results" in st.session_state and st.session_state.results:
            ...
            if not dedup_tmx:
                ...
                if not conflict_groups:
                    ...
                    st.markdown(f"<small>Segments with similar source (≥{dedup_threshold}% threshold applied to source only) but different translations. Select which translation to keep in each match.</small>", unsafe_allow_html=True)
