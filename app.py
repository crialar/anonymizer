...
def generate_clean_tmx(previews: dict, results: dict, originals: dict,
    ...
    seen_segments = []
    now = datetime.utcnow()
    processing_date = now.strftime("%Y%m%dT%H%M%SZ")
    processing_date_readable = now.strftime("%Y-%m-%d %H:%M:%S")
    ...
    for filename, file_previews in previews.items():
        for preview in file_previews:
            ...
            tu.set("creationdate", processing_date)
            
            for existing_prop in preview.get('existing_props', []):
                if existing_prop["type"] not in ("source-file", "segment-id"):
                    p = etree.SubElement(tu, "prop")
                    p.set("type", existing_prop["type"])
                    p.text = existing_prop["text"]
            
            prop_file = etree.SubElement(tu, "prop")
            prop_file.set("type", "source-file")
            prop_file.text = filename
            
            prop_seg = etree.SubElement(tu, "prop")
            prop_seg.set("type", "segment-id")
            prop_seg.text = str(preview['segment'])
            ...
            tmx_traceability.append({
                "file": filename,
                "segment": preview['segment'],
                "source": source_text,
                "target": target_text,
                "date": processing_date_readable,
                "tmx_segment": valid_count
            })
    ...
    return result, valid_count, no_anon_skipped, excluded_ids, dedup_count, dedup_details, exclusion_breakdown, tmx_traceability


def generate_changes_excel(dedup_details: list = None, exclusion_breakdown: dict = None, tmx_traceability: list = None, tmx_filename: str = "") -> bytes:
    wb = Workbook()
    ws = wb.active
    ...
    ws3 = wb.create_sheet(title="TMX Traceability")
    
    ws3.merge_cells('A1:G1')
    ws3['A1'] = "TMX TRACEABILITY - Anonymizer v6.2"
    ws3['A1'].font = title_font
    ws3['A1'].alignment = Alignment(horizontal='center')
    
    t_row = 3
    if tmx_traceability is None:
        tmx_traceability = []
    
    ws3.merge_cells(f'A{t_row}:G{t_row}')
    ws3[f'A{t_row}'] = "TMX CONTENT - Segments included in clean TMX"
    ws3[f'A{t_row}'].font = section_font
    ws3[f'A{t_row}'].fill = section_fill
    
    t_row += 1
    trace_headers = ["Source File", "Source Segment", "TMX File", "TMX Segment", "Source (anonymized)", "Target (anonymized)", "Processing Date"]
    for col, header in enumerate(trace_headers, 1):
        cell = ws3.cell(row=t_row, column=col, value=header)
        cell.font = header_font
        cell.fill = header_fill
        cell.border = border
        cell.alignment = Alignment(horizontal='center')
    ...
    t_row += 1
    t_alt_row = False
    for item in tmx_traceability:
        for col, value in enumerate([item["file"], item["segment"], tmx_filename, item["tmx_segment"], item["source"], item["target"], item["date"]], 1):
            cell = ws3.cell(row=t_row, column=col, value=value)
            cell.border = border
            if t_alt_row:
                cell.fill = alt_fill
            cell.alignment = Alignment(wrap_text=True, vertical='top')
        t_row += 1
        t_alt_row = not t_alt_row
    
    ws3.merge_cells(f'A{t_row}:G{t_row}')
    ws3[f'A{t_row}'] = f"Total: {len(tmx_traceability)} segments in TMX"
    ws3[f'A{t_row}'].font = total_font
    ws3[f'A{t_row}'].fill = total_fill
    ws3[f'A{t_row}'].border = border
...
def main():
    ...
    with tab3:
        if "results" in st.session_state and st.session_state.results:
            ...
            clean_tmx_data, valid_segments, no_anon_in_tmx, excluded_ids, dedup_count, dedup_details, exclusion_breakdown, tmx_traceability = generate_clean_tmx(
                previews=previews,
            ...
            st.session_state['tmx_traceability'] = tmx_traceability
            ...
            with col_excel:
                tmx_fname = f"Redacted_TM_{date.today().isoformat()}.tmx"
                excel_data = generate_changes_excel(
                    ...
                    tmx_traceability=st.session_state.get('tmx_traceability', []),
                    tmx_filename=tmx_fname
                )
                st.download_button(
