...
def main():
    ...
    tab1, tab2, tab_dedup, tab3 = st.tabs(["📤 Upload", "📝 Preview", "🔄 Duplicates", "📥 Download"])
        ...
        if "results" in st.session_state and st.session_state.results:
            ...
            if not dedup_tmx:
                ...
                if not dup_groups:
                    ...
                    st.markdown(f"### 🔄 {len(dup_groups)} duplicate groups found")
                    st.markdown(f"<small>Dedup threshold: {dedup_threshold}% — Select which segment to keep in each group. If no changes are made, the first occurrence by upload order is kept and later duplicates are discarded.</small>", unsafe_allow_html=True)
                    ...
                    for g_idx, group in enumerate(dup_groups):
                        ...
                        if stable_id in old_keep and old_keep[stable_id] in group_keys:
                            new_keep[stable_id] = old_keep[stable_id]
                        else:
                            new_keep[stable_id] = members[0]["key"]

                        with st.expander(
                            f"Group {g_idx + 1}: {len(members)} segments — {similarity:.0f}% similarity",
                            expanded=(g_idx < 5)
                        ):
                            options = []
                            option_keys = []
                            ...
                            for m_idx, member in enumerate(members):
                                is_kept = (option_keys[m_idx] == new_keep[stable_id])
                                if is_kept:
                                ...
                                with col1:
                                    st.markdown("**Source:**")
                                    st.markdown(
                                        f'<div style="background:#f8f9fa;border:1px solid #dee2e6;border-radius:4px;padding:0.5rem;font-size:0.9rem;">{member["source"][:500]}</div>',
                                        unsafe_allow_html=True
                                    )
                                with col2:
                                    st.markdown("**Target:**")
                                    st.markdown(
                                        f'<div style="background:#f8f9fa;border:1px solid #dee2e6;border-radius:4px;padding:0.5rem;font-size:0.9rem;">{member["target"][:500]}</div>',
                                        unsafe_allow_html=True
                                    )
    ...
    with tab3:
        if "results" in st.session_state and st.session_state.results:
            ...
            if all_have_canonical:
                ...
                use_canonical = st.checkbox("Assign canonical ID to TMX segments", value=st.session_state.get('use_canonical', False), key='use_canonical',
