...
def main():
    ...
    with st.sidebar:
        ...
        dedup_threshold = st.slider("Similarity threshold (%)", 50, 100, 100, 5, help="100% = exact duplicates only. Lower values catch similar segments too") if dedup_tmx else 100
        ...
        if "previews" in st.session_state and st.session_state.previews:
            ...
            with col_show_junk:
                show_junk_in_preview = st.checkbox(
                    ...
                    help="Show/hide junk segments in the preview (does not affect downloads)"
                )
        ...
        if "results" in st.session_state and st.session_state.results:
            ...
            if empty_total > 0:
                ...
                <div style="background-color: #e2e3e5; border: 1px solid #6c757d; border-radius: 8px; padding: 1rem; margin: 0.5rem 0;">
                    <strong>🗑️ {empty_total} empty segments in the anonymized file</strong> <small>(of {total_segs} total)</small><br>
                    <small>Excluded by filters (junk, short, heavily anonymized). Not included in clean TMX</small>
                </div>
                """, unsafe_allow_html=True)
            if dedup_count > 0:
                st.markdown(f"""
                <div style="background-color: #d1ecf1; border: 1px solid #0c5460; border-radius: 8px; padding: 1rem; margin: 0.5rem 0;">
                    <strong>🔄 {dedup_count} duplicate segments removed from TMX</strong> <small>(similarity threshold: {dedup_threshold}%)</small>
                </div>
                """, unsafe_allow_html=True)
