...
def generate_changes_excel() -> bytes:
    ...
    if has_junk:
        ...
        ws2[f'A{ex_row}'] = f"JUNK SEGMENTS - Excluded from TM (<{min_words_junk} words or only numbers/symbols)"
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("")
        st.markdown("#### Filters")
        filter_junk = st.checkbox("Exclude short segments", value=True, help="Removes junk segments: less than min. words or only numbers/symbols")
        ...
        if "results" in st.session_state and st.session_state.results:
            ...
            if filter_junk:
                ...
                if junk_count > 0:
                    ...
                    <div style="background-color: #e2e3e5; border: 1px solid #6c757d; border-radius: 8px; padding: 1rem; margin: 0.5rem 0;">
                        ...
                        <small>&lt;{min_words_junk} words or only numbers/symbols</small>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            if filter_short:
                short_count = sum(
                    ...
                    for preview in file_previews
                    if preview.get('changed', True) and not (filter_junk and is_junk_segment(preview, min_words_junk)) and segment_word_count(preview) < min_words
                )
                if short_count > 0:
                    st.markdown(f"""
